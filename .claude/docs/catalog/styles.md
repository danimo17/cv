...
| **Inset**   | `shadow-neu-inset` / `-inset-sm` | `app-input__control` (`inset`; focus `+ ring-2 ring-primary/40`; danger `ring-1 ring-danger`), radio/checkbox sense marcar (`inset-sm`), card filled (`inset`), skeleton (`inset-sm`), marquesina i banner (`inset-sm`), pista del locale switcher (`inset-sm`), nav actiu (`inset-sm`), footer (`inset-sm`)                                                                                                             |
...
`__how` (`rounded-2xl`, sobre AppCard filled), `__how-title`, `__steps`). Només layout/espai; el text és `AppText`
i el relleu el posen els primitius.

## Escala tipogràfica (`AppText`, `app-text.css`)

Tota la tipografia surt d'aquí. Cap altre CSS declara `text-*` de mida, `font-*` de pes ni color de text (excepte
els primitius que ho tenen com a part del control: `AppButton`, `AppInput__control`, `AppIcon`, `AppAlert` base
`text-sm`, `LocaleSwitcher__item`, `AppHeader__brand`).

| Variant   | Classe               | Estil                                                | On s'usa                                                                                                  |
| --------- | -------------------- | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `display` | `.app-text--display` | `text-4xl font-extrabold tracking-tight sm:text-6xl` | nom del hero (`h1`)                                                                                       |
| `h1`      | `.app-text--h1`      | `text-3xl font-extrabold tracking-tight sm:text-5xl` | títol de `/meme`                                                                                          |
| `h2`      | `.app-text--h2`      | `text-2xl font-bold tracking-tight sm:text-3xl`      | títol d'`AppSection`                                                                                      |
| `h3`      | `.app-text--h3`      | `text-xl font-semibold sm:text-2xl`                  | headline del hero                                                                                         |
| `lead`    | `.app-text--lead`    | `text-lg`                                            | salutació del hero, intro de `/meme`; amb `weight=semibold` subtítols (certificacions, títol del preview) |
| `body`    | `.app-text--body`    | `text-base leading-relaxed`                          | paràgrafs, títols d'ítem (`weight=semibold`)                                                              |
| `small`   | `.app-text--small`   | `text-sm`                                            | períodes, notes, bullets, meta, captions, labels, títol d'alerta                                          |
| `caption` | `.app-text--caption` | `text-xs`                                            | crèdit Giphy, hints, títol de `MemeCard`, badges `sm`                                                     |
| `eyebrow` | `.app-text--eyebrow` | `text-sm font-semibold tracking-wide uppercase`      | eyebrow de secció, kicker de `/meme`, títols de l'aside d'About                                           |

Modificadors ortogonals: tons `.app-text--tone-default` (hereta) / `-muted` (`text-text-muted`) / `-<tone>`
(`text-<tone>`); pesos `.app-text--normal/--medium/--semibold/--bold`; alineació `.app-text--align-start/-center/-end`;
`.app-text--truncate`.
...
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.app-link`     | `--inline` (underline a hover, `hover:text-primary` amb to default), `--nav` (`rounded-lg px-3 py-2 text-sm font-medium text-text-muted`, hover `shadow-neu-sm`), `--subtle` (`text-text-muted hover:text-text`), `--icon` (`size-9` centrat, hover `shadow-neu-sm`, active `inset-sm`), `--tone-<tone>`, `--active` (nav: `bg-surface-alt shadow-neu-inset-sm`) | —                                                                                                                                                                                                                                                                                                                                                                                                           | `transition-[color,box-shadow]`                                                                                                                                             |
| `.app-image`    | `--fit-cover/-contain`, `--radius-none/-md/-lg/-xl(3xl)/-full`, `--frame` (`shadow-neu-sm`, sense vora)                                                                                                                                                                                                                                                          | —                                                                                                                                                                                                                                                                                                                                                                                                           | `block max-w-full`                                                                                                                                                          |
| `.app-button`   | `--sm/--md/--lg` (h 8/10/12), `--solid` (fons del to + `-fg`) / `--outline` (`bg-surface ring-1 ring-<tone>/40` + text del to) / `--ghost` (`shadow-neu-none`, text del to) × `--<tone>`, `--loading` (`shadow-neu-inset`), `--block`                                                                                                                            | `__icon`                                                                                                                                                                                                                                                                                                                                                                                                    | `inline-flex rounded-lg bg-surface shadow-neu font-medium text-sm transition-shadow`; `hover:shadow-neu-sm`, `active:shadow-neu-inset`, `disabled:opacity-50 shadow-neu-sm` |
| `.app-icon`     | `--sm/--md/--lg`                                                                                                                                                                                                                                                                                                                                                 | —                                                                                                                                                                                                                                                                                                                                                                                                           |                                                                                                                                                                             |
| `.app-badge`    | `--sm/--md/--lg` (padding), `--soft` (fons `-soft` + text del to) / `--solid` (fons del to + `-fg`) × `--<tone>`                                                                                                                                                                                                                                                 | —                                                                                                                                                                                                                                                                                                                                                                                                           | `rounded-full shadow-neu-sm`; text via `AppText`                                                                                                                            |
| `.app-alert`    | `--<tone>` (fons `-soft`; icona i títol del to)                                                                                                                                                                                                                                                                                                                  | `__icon`, `__body`, `__title`, `__content`                                                                                                                                                                                                                                                                                                                                                                  | `rounded-xl p-4 text-sm shadow-neu` (sense vora)                                                                                                                            |
| `.app-card`     | `--outline` (`bg-surface shadow-neu-sm`), `--filled` (`bg-surface-alt shadow-neu-inset`), `--elevated` (`bg-surface shadow-neu-lg`), `--padding-sm/-md/-lg`                                                                                                                                                                                                      | `__header`, `__footer`                                                                                                                                                                                                                                                                                                                                                                                      | `rounded-xl transition-shadow`                                                                                                                                              |
| `.app-section`  | —                                                                                                                                                                                                                                                                                                                                                                | `__header`, `__eyebrow`, `__title`, `__subtitle`                                                                                                                                                                                                                                                                                                                                                            | `py-12 sm:py-16 scroll-mt-20`                                                                                                                                               |
| `.app-input`    | `--<type>`, `--sm/--md/--lg`, `--neutral/--danger` (control `ring-1 ring-danger`, focus `ring-danger/40`; radio/checkbox `ring-danger`, marcats `bg-danger`), `--with-icon`, `--disabled`                                                                                                                                                                        | `__label`, `__field`, `__icon`, `__control` (`h-10 rounded-lg bg-surface-alt px-3 text-sm shadow-neu-inset`, focus `ring-2 ring-primary/40`), `__group`, `__options`, `__option`, `__radio` (`appearance-none rounded-full`), `__checkbox` (`appearance-none rounded-sm`): `size-4 bg-surface-alt shadow-neu-inset-sm`, `:checked` `bg-primary shadow-neu-sm` + `::after` (punt / marca en `-fg`), `__hint` | textarea/multiselect `h-auto min-h-24`                                                                                                                                      |
| `.app-skeleton` | `--text`, `--image`, `--circle`                                                                                                                                                                                                                                                                                                                                  | —                                                                                                                                                                                                                                                                                                                                                                                                           | `animate-pulse rounded-lg bg-surface-alt shadow-neu-inset-sm motion-reduce:animate-none`                                                                                    |
| `.app-marquee`  | `--slow/--normal/--fast` (70/40/20 s via `--app-marquee-duration`), `--pause`, `--<tone>` (fons `-soft`), `--sm/--md/--lg`                                                                                                                                                                                                                                       | `__track` (animació `app-marquee-scroll`, `translateX(0 → -50%)`), `__copy`                                                                                                                                                                                                                                                                                                                                 | franja `shadow-neu-inset-sm`; `prefers-reduced-motion`: sense animació, còpia oculta                                                                                        |
...
| `.source-banner`      | —                                                                    | `relative z-30` (el relleu el posa `AppMarquee`); `__item` (`inline-flex gap-2 px-6`), `__icon` (`opacity-60`)                                                                                                                                                |
| `.theme-toggle`       | —                                                                    | (sobre `.app-button` ghost) `size-10 rounded-full bg-surface px-0 shadow-neu`, hover `neu-sm`, active `neu-inset`                                                                                                                                             |
...
| `.meme-card`          | `--selected` (`ring-2 ring-primary shadow-neu-inset`, també a hover) | (sobre `.app-button` ghost) `rounded-xl bg-surface shadow-neu`, hover `neu-lg`, active `neu-inset`; `__image`, `__title` (overlay `bg-secondary/80`, visible a hover/focus/selected)                                                                          |
...
4. **Afegir un to**: 6 valors (`-`, `-fg`, `-soft` × clar/fosc) a `tokens.css`, entrada a `TONES`
   (`app/types/ui.ts`) i modificadors `--<to>` a `app-button`, `app-badge`, `app-alert`, `app-link`, `app-marquee`,
   `app-text`. Comprova que `<to>` sobre `surface` i `-fg` sobre `<to>` donen ≥ 4.5:1.
5. **Canviar la tipografia**: `app-text.css` (variants) o `tokens.css` (`--font-*`). Cap altre fitxer.
...
8. **Nova secció/pàgina**: només layout (grid, gaps, marges) al seu `<kebab>.css`; text amb `AppText`, enllaços
   amb `AppLink`, imatges amb `AppImage`; el relleu el posa `AppCard` o el primitiu corresponent.
