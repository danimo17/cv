...
- **Al template només classes pròpies.** Cap utilitat Tailwind (`vue/no-restricted-class` bloqueja
  ...
  (`CustomText`, `CustomLink`, `CustomSection`…) sí que és correcte: el fill la fusiona a la seva arrel.
- **Tipografia només via `CustomText`.** `custom-text.css` és l'únic fitxer amb mides (`text-xs … text-6xl`), pesos i
  colors de text per a contingut. Els CSS de seccions i layout només tenen layout, espai, fons i relleu. Excepcions
  documentades a `catalog/styles.md`: text propi dels controls (`CustomButton`, `CustomInput__control`, `CustomIcon`,
  base d'`CustomAlert`, `LocaleSwitcher__item`, `AppHeader__brand`).
...
- **Què va enfonsat (inset).** Tot el que és "contenidor" o s'escriu: controls d'`CustomInput` (`shadow-neu-inset`),
...
- **Moviment.** Tota transició i animació respecta `prefers-reduced-motion: reduce` (`base.css` talla les
  transicions; `custom-marquee.css` i `custom-skeleton.css` treuen l'animació).
