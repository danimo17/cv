...
| 2   | Given `www.danimorales.dev`, When es fa una petició HTTPS, Then respon (redirect o mateixa app)                                                                               | ✅ fet — Custom Domain afegit al Worker `cv`; `curl` amb `--resolve` a la IP de Cloudflare → 200 (DNS local encara amb caché de l'NXDOMAIN previ, propagació normal)                                                |
| 3   | Given el job de Dependabot "npm_and_yarn Update #2", When torna a córrer, Then no falla                                                                                       | ✅ fet — `packageManager` baixat a `pnpm@11.17.0` (evita l'autodescàrrega del binari natiu que fallava al sandbox de Dependabot); `pnpm install --frozen-lockfile` + `pnpm gate` verds localment amb la nova versió |
...
- Cap. D1-D5 decidides (034-038). Dependabot: l'usuari ha triat downgrade de `packageManager` (vs esperar el
  reintent automàtic) — aplicat.
